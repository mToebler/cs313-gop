...
<div class="container">
   ...
   <?php
      ...
      if(isset($search_text) && strlen($search_text)) {
         ...
         $qstring = "select i.id as id, i.name as item, i.description as idesc, m.name as cat, l.description as location, ip.start_date as start_date, ip.returned_date as returned_date from items i join meta_item mi on i.id = mi.item_id join metas m on mi.meta_id = m.id join locations l on i.location_id = l.id left join item_possession ip on i.id = ip.id where LOWER(i.name) like LOWER('%$search_text%') OR LOWER(i.description) like LOWER('%$search_text%') order by item, returned_date desc;";
      } else {
         $qstring = 'select i.id as id, i.name as item, i.description as idesc, m.name as cat, m.id as mid, l.name as lname, l.description as location, ip.returned_date as returned_date, ip.start_date as start_date from items i join meta_item mi on i.id = mi.item_id join metas m on mi.meta_id = m.id join locations l on i.location_id = l.id left join item_possession ip on i.id = ip.item_id order by item, returned_date desc';
      }
